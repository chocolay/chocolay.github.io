...
//console.log(pageX,pageY,'on')
            mouseOnBall.set(
                (pageX - _this.screen.width/2  - _this.screen.left)/(_this.screen.width/2),
                -(pageY -_this.screen.height/2 - _this.screen.top )/(_this.screen.height/2),
                0.0
            );
            var length = mouseOnBall.length();
            if ( _this.noRoll ) {
                mouseOnBall.z = (length<Math.SQRT1_2)?Math.sqrt(1-length*length):0.5/length;
            } else if (length > 1.0) {
 ...
 //   d3.select("#renderDiv").on("mousemove",function() {console.log(d3.mouse(this)+'hi') })
...
//temporary to see how to transfer to d3
